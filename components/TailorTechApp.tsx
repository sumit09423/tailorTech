...
type PageKey = "home" | "courses" | "detail" | "login" | "profile";
type NavFn = (t: PageKey, id?: string) => void;
type UserState = {
  name: string;
  email: string;
  phone?: string;
...
type ProfileTabKey = "profile" | "courses" | "settings";
...
/* ─── SHARED ─── */
function Nav({page,nav,w,user,onLogout}:{page:PageKey;nav:NavFn;w:number;user:UserState|null;onLogout:()=>void}){
  ...
  const m=w<768;const go=(t:PageKey,id?:string)=>{setMo(false);nav(t,id)};const sc=(id:string)=>{setMo(false);nav("home");setTimeout(()=>document.getElementById(id)?.scrollIntoView({behavior:"smooth"}),120)};
  return<>
    <nav style={{position:"fixed",top:0,width:"100%",zIndex:1000,padding:m?".7rem 5%":".85rem 4%",display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(255,253,251,.92)",backdropFilter:"blur(16px)",borderBottom:"1px solid rgba(0,0,0,.04)",boxShadow:sh?"0 3px 18px rgba(0,0,0,.05)":"none",transition:"all .3s"}}>
      ...
      {!m?<div style={{display:"flex",gap:"1.5rem",alignItems:"center"}}>
        {([["home","Home"],["courses","Courses"]] as const).map(([k,l])=><a key={k} onClick={()=>go(k)} style={{color:page===k?"#5A8A6C":"#6B7C8F",fontSize:".84rem",fontWeight:page===k?700:500,cursor:"pointer",borderBottom:page===k?"2px solid #7BAE8E":"2px solid transparent",paddingBottom:3}}>{l}</a>)}
...
function Foot({nav,w}:{nav:NavFn;w:number}){const m=w<640;const t=w<1024;
  const L=({children,onClick}:{children:ReactNode;onClick?:()=>void})=><a onClick={onClick} style={{display:"block",color:"rgba(255,255,255,.4)",fontSize:".84rem",marginBottom:".45rem",cursor:"pointer"}}>{children}</a>;
...
function CTA({nav,w}:{nav:NavFn;w:number}){const m=w<640;return<section style={{background:"linear-gradient(135deg,#7BAE8E,#5A8A6C)",padding:m?"2.5rem 5%":"4rem 6%",textAlign:"center",position:"relative",overflow:"hidden"}}>
...
function SH({label,title,sub,light,w}:{label:string;title:string;sub?:string;light?:boolean;w:number}){const m=w<640;return<Reveal><div style={{textAlign:"center",marginBottom:m?"1.8rem":"2.8rem"}}>
...
function CC({c,nav,delay=0}:{c:CourseRow;nav:NavFn;delay?:number}){const[h,sH]=useState(false);return<Reveal delay={delay} style={{height:"100%"}}>
...
function Toast({msg}:{msg:string}){if(!msg)return null;return<div className="toast">{msg}</div>}

/* ═══════ LOGIN / REGISTER ═══════ */
function LoginPage({nav,w,onLogin}:{nav:NavFn;w:number;onLogin:(u:UserState)=>void}){
  const m=w<640;const[mode,setMode]=useState("login");
  const[form,setForm]=useState({name:"",email:"",phone:"",password:""});
  const[error,setError]=useState("");
  const upd=<K extends keyof typeof form>(k:K)=>(e:ChangeEvent<HTMLInputElement>)=>setForm({...form,[k]:e.target.value});
  const submit=()=>{
    setError("");
...
/* ═══════ PROFILE PAGE ═══════ */
function ProfilePage({nav,w,user,setUser,onLogout,enrolled: _enrolled,onUnenroll}:{nav:NavFn;w:number;user:UserState;setUser:(u:UserState)=>void;onLogout:()=>void;enrolled:string[];onUnenroll:(id:string)=>void}){
  ...
  const upd=<K extends keyof typeof form>(k:K)=>(e:ChangeEvent<HTMLInputElement|HTMLTextAreaElement>)=>setForm({...form,[k]:e.target.value});
...
/* ═══════ HOME (compact) ═══════ */
function Home({nav,w,user}:{nav:NavFn;w:number;user:UserState|null}){const m=w<640;const t=w<1024;return<>
...
/* ═══════ COURSES ═══════ */
function Courses({nav,w}:{nav:NavFn;w:number}){const m=w<640;const t=w<1024;const[filterKey,sF]=useState<string>("all");const fl=filterKey==="all"?COURSES:COURSES.filter(c=>c.cat===filterKey);
  return<><section style={{padding:m?"5rem 5% 1.5rem":"6.5rem 6% 2rem",background:"#FFF8F0",textAlign:"center"}}><div style={{maxWidth:620,margin:"0 auto"}}><div style={{marginBottom:".8rem",fontSize:".75rem"}}><a onClick={()=>nav("home")} style={{color:"#6B7C8F",cursor:"pointer"}}>Home</a><span style={{color:"#6B7C8F",margin:"0 .3rem"}}>›</span><span style={{color:"#1E2A3A",fontWeight:700}}>All Courses</span></div><h1 style={{fontFamily:"'DM Serif Display',serif",fontSize:m?"1.5rem":"clamp(1.7rem,2.8vw,2.5rem)",color:"#1E2A3A",marginBottom:".4rem"}}>All <em style={{fontStyle:"italic",color:"#7BAE8E"}}>Courses</em></h1><p style={{fontSize:m?".82rem":".95rem",color:"#6B7C8F"}}>Explore our complete software development catalog.</p></div></section>
    <div style={{padding:m?".8rem 5%":"1rem 6%",background:"#FFFDFB",borderBottom:"1px solid rgba(0,0,0,.04)",position:"sticky",top:m?44:52,zIndex:100,backdropFilter:"blur(10px)"}}><div style={{maxWidth:1200,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",gap:".5rem",flexWrap:"wrap"}}><div style={{display:"flex",gap:m?".2rem":".35rem",flexWrap:"wrap"}}>{FILTERS.map(fi=><button key={fi.key} onClick={()=>sF(fi.key)} style={{padding:m?".3rem .55rem":".4rem .9rem",borderRadius:50,fontSize:m?".65rem":".74rem",fontWeight:700,border:filterKey===fi.key?"2px solid #7BAE8E":"2px solid rgba(0,0,0,.05)",background:filterKey===fi.key?"#7BAE8E":"#fff",color:filterKey===fi.key?"#fff":"#6B7C8F",cursor:"pointer",fontFamily:"'Mulish',sans-serif"}}>{fi.label}</button>)}</div><span style={{fontSize:".8rem",color:"#6B7C8F"}}><strong style={{color:"#1E2A3A"}}>{fl.length}</strong> courses</span></div></div>
...
/* ═══════ DETAIL ═══════ */
function Detail({nav,cid,w,user,onEnroll}:{nav:NavFn;cid:string;w:number;user:UserState|null;onEnroll:(id:string)=>void}){const c=COURSES.find(x=>x.id===cid)||COURSES[0];const m=w<640;const t=w<1024;
  const[tab,sTab]=useState("curriculum");const[oM,sOM]=useState<number[]>([0]);const[oF,sOF]=useState<number[]>([]);const[toast,setToast]=useState("");
  const tM=(i:number)=>sOM(p=>p.includes(i)?p.filter(x=>x!==i):[...p,i]);const tF=(i:number)=>sOF(p=>p.includes(i)?p.filter(x=>x!==i):[...p,i]);
  const disc=Math.round((1-parseInt(c.price.replace(",",""))/parseInt(c.original.replace(",","")))*100);
  const isEnrolled=Boolean(user&&(user.enrolled||[]).includes(c.id));
...
/* ═══════ APP ═══════ */
export default function TailorTechApp() {
  const [pg, sPg] = useState<PageKey>("home");
  const [cid, sCid] = useState("fullstack-web");
  const w = useW();
  const [user, setUser] = useState<UserState | null>(null);
  const nav = useCallback((t: PageKey, id?: string) => {
    if (t === "detail") {
      sCid(id || "fullstack-web");
      sPg("detail");
    } else sPg(t);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, []);
  const handleLogin = (u: UserState) => setUser(u);
  const handleLogout = () => {
    setUser(null);
    sPg("home");
  };
  ...
  const handleUnenroll = (id: string) => {
    ...
    setUser({
      ...user,
      enrolled: (user.enrolled || []).filter((x) => x !== id),
    });
  };
